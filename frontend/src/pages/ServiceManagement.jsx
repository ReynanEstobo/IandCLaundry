...
export default function ServiceManagement() {
  ...
  async function saveRecipe(event) {
    ...
    const payload = { ...recipeForm, quantity_per_unit: Number(recipeForm.quantity_per_unit) }
    const result = await runQuery('service_inventory_requirements', editingRecipe
      ...
      : { operation: 'insert', payload, returning: true })
    if (result.error) return toast.error(result.error.message)
    toast.success(editingRecipe ? 'Inventory requirement updated.' : 'Inventory requirement added.')
  ...
  return <div className="settings-page">
    ...
    <div className="card settings-card">
      <div className="settings-card-header"><div><h3>Service Inventory Requirements</h3><p>For each service, choose the branch inventory item and the exact amount consumed per kilogram, per piece, or once per service item. This is the only configuration used for automatic service deductions; the old inventory-wide usage-per-load value is not used.</p></div></div>
      {recipeSetupError && <div className="alert alert-warning" role="status">{recipeSetupError}</div>}
      <form onSubmit={saveRecipe}><div className="form-row"><div className="form-group"><label>Branch *</label><select className="form-control" required value={recipeForm.branch_id} onChange={(e) => setRecipeForm((f) => ({ ...f, branch_id: e.target.value, inventory_item_id: '' }))}><option value="">Select branch</option>{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></div><div className="form-group"><label>Service *</label><select className="form-control" required value={recipeForm.service_type_id} onChange={(e) => setRecipeForm((f) => ({ ...f, service_type_id: e.target.value }))}><option value="">Select service</option>{services.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</select></div><div className="form-group"><label>Inventory item *</label><select className="form-control" required value={recipeForm.inventory_item_id} onChange={(e) => setRecipeForm((f) => ({ ...f, inventory_item_id: e.target.value }))}><option value="">Select item</option>{selectedInventory.map((item) => <option key={item.id} value={item.id}>{item.name} ({item.unit})</option>)}</select></div></div><div className="form-row"><div className="form-group"><label>Usage basis *</label><select className="form-control" value={recipeForm.usage_basis} onChange={(e) => setRecipeForm((f) => ({ ...f, usage_basis: e.target.value }))}><option value="per_kg">Per kilogram</option><option value="per_piece">Per piece</option><option value="per_order">Once per service item</option></select></div><div className="form-group"><label>Amount consumed *</label><input className="form-control" required min="0.0001" step="0.0001" type="number" value={recipeForm.quantity_per_unit} onChange={(e) => setRecipeForm((f) => ({ ...f, quantity_per_unit: e.target.value }))} /></div></div><button className="btn btn-primary" type="submit"><Save size={16} /> {editingRecipe ? 'Update requirement' : 'Add requirement'}</button>{editingRecipe && <button className="btn btn-secondary" type="button" style={{ marginLeft: 8 }} onClick={() => { setEditingRecipe(null); setRecipeForm(blankRecipe) }}>Cancel</button>}</form>
      <div className="table-wrapper" style={{ marginTop: 18 }}><table><thead><tr><th>Branch</th><th>Service</th><th>Inventory item</th><th>Formula</th><th /></tr></thead><tbody>{requirements.map((requirement) => <tr key={requirement.id}><td>{branchNames.get(requirement.branch_id) || 'Branch'}</td><td>{names.get(requirement.service_type_id) || 'Service'}</td><td>{itemNames.get(requirement.inventory_item_id) || 'Item'}</td><td>{Number(requirement.quantity_per_unit)} × {requirement.usage_basis === 'per_order' ? 'once per service item' : requirement.usage_basis.replace('_', ' ')}</td><td><button className="btn btn-secondary btn-sm" onClick={() => { setEditingRecipe(requirement); setRecipeForm({ branch_id: requirement.branch_id, service_type_id: requirement.service_type_id, inventory_item_id: requirement.inventory_item_id, usage_basis: requirement.usage_basis, quantity_per_unit: requirement.quantity_per_unit, is_active: requirement.is_active }) }}><Edit2 size={14} /></button> <button className="btn btn-danger btn-sm" onClick={() => removeRecipe(requirement.id)}><Trash2 size={14} /></button></td></tr>)}</tbody></table></div>
    </div>
  </div>
}
